...
| Step 04 | 建立 AI/BI Dashboard 
| Step 05 | 建立 Genie Space 
| Step 06 | 建立 Evaluation Dataset 
| Step 07 | 執行 Evaluation 
...
✅ **完成確認**：Dashboard 顯示 6 張圖表，資料正常呈現

---

## Step 05｜建立 Genie Space（遵照最佳實踐，預留調整空間）
...
> 📌 **預期觀察**：「電視購物和網路購物的業績比較」這道題很可能評估結果為 Bad — 因為 Genie 不知道 `media_gb` 的代碼對應（'01'=電視購物, '07'=網路購物）。這個缺口將在下半場透過使用者反饋 → Monitor → 優化的流程來修正。

> 💡 **討論點**：Fail 的題目是很好的教學素材，帶大家看 Genie 生成的 SQL 哪裡出錯，並說明下半場將如何透過真實使用者反饋來驅動改善。
...
各角色請找到上半場倉儲人員建立的 **momo電商分析儀表板**，依照以下場景查看對應圖表：
...
2. 哪些商品分類的成長速度超過平均？
3. 從 Dashboard 數字來看，你會選擇用 Genie 深挖哪個問題？
...
各角色在 Databricks One 找到對應的Genie Space，進行以下問答。
...
> 💡 **討論點：三種不同性質的失敗**
>
> Monitor 收到的反饋來自兩個來源（Step 7 Evaluation + Step 10 使用者差評），共呈現三種失敗類型：
>
> | 來源 | 問題 | 失敗原因 | 性質 | 修復方式 |
> |------|------|----------|------|----------|
> | Step 7 Evaluation | 電視購物和網路購物的業績比較 | Genie 不知道 `media_gb` 代碼對應，邏輯錯誤 | **術語不明確** | Instructions + Example SQL |
> | Step 7 Evaluation | 1P 和 3P 各自的平均客單價 | 計算正確但輸出格式不穩定，SQL-diff 誤判為 Bad | **格式不穩定**（非真正錯誤） | Example SQL 固定輸出格式 |
...
> 💡 **討論點**：這個從「反饋 → Monitor → 優化 → 驗證」的完整循環，就是 Genie 在企業環境中持續運作的健康模式。兩題揭示了兩種截然不同的失敗類型：**術語不明確**（用 Example SQL 修正）vs **資料根本不存在**（用 Instructions 劃定邊界）。
...
> ⚠️ **注意**：請勿修改上半場倉儲人員建立的原始 Dashboard，避免影響他人查看。
